(function () {
  ...
  const APP_URL = window.APP_URL || 'https://edmond-mouthier-ununiquely.ngrok-free.dev';
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  btn.addEventListener("click", async () => {
    ...
    const res = await fetch(
      `https://edmond-mouthier-ununiquely.ngrok-free.dev/referral/link?customerId=${window.LOGGED_IN_CUSTOMER_ID}&shopDomain=${encodeURIComponent(shopDomain)}`,
      {
        headers: {
    ...
    if (data.customer) {
      // Example: You can use this data to customize the UI
      console.log(`Hello ${data.customer.first_name || 'Generic Customer'}!`);
    }
